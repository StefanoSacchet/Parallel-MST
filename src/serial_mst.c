...
tot_mst_weight_t serial_mst(struct Graph *graph) {
  ...
  for (graph_size_t i = 0; i < V && edges_mst < V - 1; i++) {
    // Initialize cheapest array
    for (graph_size_t i = 0; i < V; i++) {
      cheapest[i].weight = -1;
    }

    // Traverse through all edges and update cheapest of every component
    for (graph_size_t i = 0; i < E; i++) {
      Edge_t current_edge = edge[i];
      edge_t set1 = find(subsets, current_edge.src);
      edge_t set2 = find(subsets, current_edge.dest);
    ...
    for (graph_size_t i = 0; i < V; i++) {
      ...
      if (cheapest[i].weight != -1) {
        ...
        edge_t set1 = find(subsets, edge.src);
        edge_t set2 = find(subsets, edge.dest);
